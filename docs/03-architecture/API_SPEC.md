...
| `features.suministros_con_vector` | integer | Suministros no excluidos por la Etapa 1 que recibieron un `FeatureVector` esta corrida (AI_ENGINE_SPEC.md §6). Puede ser menor que `informe.total_suministros - informe.suministros_excluidos` cuando el ÚNICO período de un suministro en este lote fue, a su vez, uno de los lados de un par conflictivo de la Etapa 2 (ver §6.3) |
| `features.cold_starts` | integer | Cuántos de esos vectores tienen `is_cold_start = true` (menos de 3 períodos de historia disponible, DEC-007) |
| `features.con_periodos_conflictivos` | integer | Cuántos de esos vectores tienen `has_conflicted_periods = true` (el suministro aparece en `duplicidades.periodos_conflictivos`, aunque el período conflictivo no sea el actual) |
