...
11. **Alinear `clientes`/`suministros`/`lecturas`' DTOs de importación a `extra="forbid"`, por consistencia.** `LoteImportItem` (`backend/src/energia/contexts/consumos/presentation/schemas.py`) pasó de `extra="ignore"` (default de Pydantic) a `extra="forbid"`: una clave desconocida en el payload (`estado`, o un campo mal tipeado como `canditad_registros`) se rechaza ahora de forma estructural, nombrando la clave ofensora, en lugar de descartarse en silencio sin dejar rastro. `ClienteImportItem`, `SuministroImportItem` y `LecturaImportItem` siguen usando `extra="ignore"` — evaluar si conviene alinearlos al mismo `extra="forbid"` queda pendiente; no se modificaron en esta pasada.

12. **Máquina de estados de `Lote`: ¿`Error` es terminal o admite reintento (`Error` → `Procesando`)?** Decisión de negocio pendiente antes de implementar el motor de procesamiento. `ALLOWED_TRANSITIONS` (`backend/src/energia/contexts/consumos/domain/lote.py`) asume hoy que `Error` es terminal (sin transición de salida), la misma decisión que ya toma para `Procesado` — pero a diferencia de `Procesado` (un estado de éxito, terminal por diseño), que un lote que falló no pueda reintentarse es una suposición de negocio no confirmada por ningún documento canónico, no una regla derivada de RD-010.

13. **Sin vía de API para corregir/retractar un período de consumo mal ingresado (clave natural nueva ⇒ fila duplicada activa)**; definir endpoint de baja lógica o proceso de corrección antes de cargar históricos reales. Distinto de la superposición parcial no detectada de RD-017 (`docs/03-architecture/API_SPEC.md`, "Contexto: Gestión de Consumos"): un `fecha_inicio`/`fecha_fin` corregido por reimportación cambia la clave natural compuesta `(numero_suministro, fecha_inicio, fecha_fin)`, así que la reimportación crea una fila nueva en lugar de reemplazar la original con el período incorrecto — que queda activa, sin ninguna vía de API (DELETE o baja lógica) para retractarla. Hoy la única forma de corregirlo es un soft-delete manual por SQL directo.
