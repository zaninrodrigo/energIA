...
| `docs/03-architecture/API_SPEC.md` | Especificación de la API REST del backend | En progreso (Clientes, Suministros y Consumos/Lectura/Lote documentados) |
...
10. **`uq_consumos_suministro_periodo` no es un índice único parcial, a diferencia de sus pares.** `uq_clientes_numero_cliente`, `uq_suministros_numero_suministro` y `uq_lecturas_suministro_fecha` son todos índices únicos parciales (`WHERE deleted_at IS NULL`), lo que permite reimportar una clave natural soft-deleted como una fila nueva sin conflicto. `uq_consumos_suministro_periodo` (`docker/postgres/init/01_schema.sql`, tabla `consumos`) es en cambio un `CONSTRAINT ... UNIQUE` simple, sin esa cláusula: reimportar el mismo `(suministro_id, fecha_inicio, fecha_fin)` después de un soft-delete chocaría contra la fila soft-deleted en lugar de crear una nueva, rompiendo la idempotencia de importación que sí tienen clientes, suministros y lecturas. Debe convertirse a un índice único parcial antes de que la historia de usuario US-004 (Consumo) se implemente.
